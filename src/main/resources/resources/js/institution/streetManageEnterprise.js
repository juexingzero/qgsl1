...
function allCancel(){
	...
	if (confirm("确认要全部取消吗？")) {
        Ajax({
            type: "post",
            url: "/web/v1/institution/cancelAllEnterpriseSelect?institution_id="+institution_id,
            success: function (msg) {
                if (errorCheck(msg)) {
                	if(msg.data != null){
                		alert(msg.data);
...
function cancel(gridId,row_uid){
	...
	if (row && confirm("确定要取消选择吗？")) {
        Ajax({
            type: "post",
            url: "/web/v1/institution/cancelEnterpriseSelect?institution_id="+row.institution_id,
            success: function (msg) {
                if (errorCheck(msg)) {
                	if(msg.data != null){
                		alert(msg.data);
...
function select(gridId,row_uid){
	...
	if (row && confirm("确定要选择企业吗？")) {
        Ajax({
            type: "post",
            url: "/web/v1/institution/selectEnterprise?institution_id="+row.institution_id+"&main_institution="+institution_id,
            success: function (msg) {
                if (errorCheck(msg)) {
                	if(msg.data != null){
                		alert(msg.data);
